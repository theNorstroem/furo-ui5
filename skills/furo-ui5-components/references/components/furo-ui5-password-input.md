...
tags: [password, masked, secret, input, credentials, form]
category: Form
use-when: Use for entering passwords or other sensitive masked values.
---

# furo-ui5-password-input

> Masked single-line input for password entry.
...
| `password-hidden` | `CustomEvent<undefined>` | Fired when the password is hidden, after calling the hide() method. |
| `password-showed` | `CustomEvent<undefined>` | Fired when the password is shown, after calling the show method. |
