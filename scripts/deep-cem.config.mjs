      ...
      analyzePhase({ ts, node, moduleDoc, context }) {
        switch (node.kind) {
          case ts.SyntaxKind.ClassDeclaration:
            ...
            // AI retrieval metadata. These feed `npm run audit:metadata` and the
            // skill reference docs generated by `npm run skills:update`.
            // Note the tag/field name mismatch: @usecase -> useCase, @related -> relatedComponents.
            node.jsDoc?.forEach(docBlock => {
              docBlock?.tags?.forEach(tag => {
                const tagName = tag?.tagName?.escapedText;

                if (tagName === "keywords") {
                  currDocDeclaration.keywords = tag.comment
                    ?.split(",")
                    .map(k => k.trim())
                    .filter(k => k.length > 0);
                }
                if (tagName === "category") {
                  currDocDeclaration.category = tag.comment?.trim();
                }
                if (tagName === "usecase") {
                  currDocDeclaration.useCase = tag.comment?.trim();
                }
                if (tagName === "related") {
                  currDocDeclaration.relatedComponents = tag.comment
                    ?.split(",")
                    .map(k => k.trim())
                    .filter(k => k.length > 0);
                }
