...
export default {
  globs: ["./src/elements/**/*.ts", "./src/types/*.ts", "./src/type-renderers/**/*.ts"],
  exclude: ["./dist/**/*", "./src/**/*.spec.ts", "./src/**/*.stories.ts"],
  outdir: "./",
  paths: { "@/*": ["./src/*"] },
  dev: false,
  litelement: true,
              ...
              switch (member.kind) {
                ...
                case ts.SyntaxKind.PropertyDeclaration:
                  member?.jsDoc?.forEach(docBlock => {
                    ...
                    const tsProgramClassNode = sourceFile.statements.find(
                      statement => ts.isClassDeclaration(statement) && statement.name?.text === node.name?.text
                    );
                    ...
                    const list = typeChecker
                      .getTypeAtLocation(tsProgramMember)
                      .types?.map(t => {
                        if (t.value) {
                          return `"${t.value}"`;
                        }
