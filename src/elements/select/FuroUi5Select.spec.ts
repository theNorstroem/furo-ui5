/**
 * Spec for `FuroUi5Select`. Mirrors the `[TEMPLATE]` describe-blocks from
 ...
 *   - tag / class swap to `furo-ui5-select` / `FuroUi5Select`
 *   - FAT attribute list reduced to `tooltip` (the only attribute select maps)
 *   - constraints test drops `max_length` (select doesn't apply it)
 *   - UI → model writes are driven via `change` only (select doesn't listen to `input`)
 *   - lifecycle block skipped (select registers no listeners in lifecycle hooks)
 * Blocks tagged `[element-specific]` cover select-only surface: `clear()`,
...
/**
 * Local helper: select-only UI → model driver. The native UI5 Select listens on
 * `change` (not `input`), so we set the value and dispatch only `change`.
 */
const fireChange = (el: FuroUi5Select, value: string): void => {
  el.value = value;
  el.dispatchEvent(new Event("change", { bubbles: true, composed: true }));
};

describe("FuroUi5Select", () => {
  // ───────────────────────────────────────────────────────────────────────
  // [TEMPLATE] Element identity & accessibility
  // ───────────────────────────────────────────────────────────────────────
  describe("element identity & a11y [TEMPLATE]", () => {
    let el: FuroUi5Select;
    let elLocator: LocatorSelectors;

    beforeAll(async () => {
      el = await fixture(html` <furo-ui5-select accessible-name="name" data-testid="test"></furo-ui5-select> `);
    ...
    it("should be a furo-ui5-select element", () => {
      // keep this test on top, so you can recognize a wrong assignment
      assert.equal(el.nodeName.toLowerCase(), "furo-ui5-select");
    });
  ...
  describe("default model state [TEMPLATE]", () => {
    ...
    beforeEach(async () => {
      el = await fixture(html`<furo-ui5-select></furo-ui5-select>`);
    });
  ...
  describe("FAT attribute mapping [TEMPLATE]", () => {
    ...
    it("pre-set accessible-name wins over model __label", async () => {
      const el: FuroUi5Select = await fixture(html`<furo-ui5-select accessible-name="preset"></furo-ui5-select>`);
      const model = createFatString();
      el.bindData(model);
  ...
  describe("bindOptions / optionsModel [element-specific]", () => {
    ...
    it("bindOptions(undefined) is a no-op", () => {
      ...
      assert.equal(el.querySelectorAll("furo-ui5-option").length, 0);
    });

    it("renders one furo-ui5-option per option from the model", () => {
      ...
      const items = el.querySelectorAll("furo-ui5-option");
    ...
    it("re-renders when the underlying array changes (array-changed)", () => {
      ...
      el.bindOptions(options);
      assert.equal(el.querySelectorAll("furo-ui5-option").length, 2);
  ...
  describe("renderOptionList / optionList [element-specific]", () => {
    ...
    it("renders one furo-ui5-option per SelectOption with value + text", () => {
      ...
      const items = el.querySelectorAll("furo-ui5-option");
      assert.equal(items.length, 2);
      assert.equal((items[0] as HTMLElement & { value?: string }).value, "1");
      assert.equal((items[0] as HTMLElement).innerText, "A");
      assert.equal((items[0] as HTMLElement & { additionalText?: string }).additionalText, "extra");
      assert.equal((items[1] as HTMLElement & { value?: string }).value, "2");
      assert.equal((items[1] as HTMLElement).innerText, "B");
