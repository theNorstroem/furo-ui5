/**
 * Spec for `FuroUi5Checkbox`, derived from the canonical template
 * `src/elements/text-input/FuroUi5TextInput.spec.ts` (with the boolean
 * adaptations established in `src/elements/switch/FuroUi5Switch.spec.ts`).
 *
 ...
 *
 * Checkbox-specific notes vs. the TextInput template:
 *   - The value property is `checked` (boolean), not `value` (string).
 *   - Supported model types are BOOLEAN | FuroFatBool | BoolValue.
 *   - `FuroUi5Checkbox` composes `FieldNodeValueState`, so the value-state
 *     subtests from the template are retained (unlike `FuroUi5Switch`).
 *   - The `read_only` constraint lands on `el.readonly` (see
 *     `FuroUi5Checkbox.handleConstraints`).
 *   - `FuroUi5Checkbox` registers no lifecycle-bound listeners, so the
...
import type { FuroUi5Checkbox } from "./FuroUi5Checkbox";
...
describe("FuroUi5Checkbox", () => {
  // ───────────────────────────────────────────────────────────────────────
  // [TEMPLATE] Element identity & accessibility
  // ───────────────────────────────────────────────────────────────────────
  describe("element identity & a11y [TEMPLATE]", () => {
    let el: FuroUi5Checkbox;
    let elLocator: LocatorSelectors;

    beforeAll(async () => {
      el = await fixture(html` <furo-ui5-checkbox accessible-name="name" data-testid="test"></furo-ui5-checkbox> `);
    ...
    it("should be a furo-ui5-checkbox element", () => {
      // keep this test on top, so you can recognize a wrong assignment
      assert.equal(el.nodeName.toLowerCase(), "furo-ui5-checkbox");
    });
  ...
  describe("default model state [TEMPLATE]", () => {
    ...
    beforeEach(async () => {
      el = await fixture(html`<furo-ui5-checkbox></furo-ui5-checkbox>`);
    });
  ...
  describe("model → UI value sync [TEMPLATE]", () => {
    ...
    it("reads an unset BoolValue as false, not null", async () => {
      ...
      await delay(10);
      assert.equal(el.shadowRoot!.querySelector('[part="root"]')!.getAttribute("aria-checked"), "false");
    });
  ...
  //
  // Checkbox composes `FieldNodeValueState`, so the value-state subtests are
  // retained. `parent-readonly-set/unset` and the `read_only` constraint both
  // land on `el.readonly` (UI5 CheckBox exposes `readonly`; see
  // `FuroUi5Checkbox.handleConstraints`).
  // ───────────────────────────────────────────────────────────────────────
  describe("model-driven state [TEMPLATE]", () => {
    ...
    it("applies field constraints (required, read_only) on bind", () => {
      ...
      el.bindData(model);
      assert.equal(el.required, true);
      // FuroUi5Checkbox maps read_only → readonly (UI5 CheckBox exposes readonly)
      assert.equal(el.readonly, true);
    });
  });
  ...
  //
  // FuroUi5Checkbox maps "readonly", "disabled", and "required"
  ...
  describe("FAT attribute mapping [TEMPLATE]", () => {
    ...
    it("applies 'readonly' FAT attribute to el.readonly", async () => {
      const el: FuroUi5Checkbox = await fixture(html`<furo-ui5-checkbox></furo-ui5-checkbox>`);
    ...
    it("pre-set HTML 'disabled' wins over FAT attribute", async () => {
      const el: FuroUi5Checkbox = await fixture(html`<furo-ui5-checkbox disabled></furo-ui5-checkbox>`);
    ...
    it("pre-set accessible-name wins over model __label", async () => {
      const el: FuroUi5Checkbox = await fixture(html`<furo-ui5-checkbox accessible-name="preset"></furo-ui5-checkbox>`);
      const model = createFatBool();
      el.bindData(model);
