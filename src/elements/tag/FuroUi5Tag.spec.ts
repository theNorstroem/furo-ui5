/**
 * Spec for furo-ui5-tag, adapted from the `FuroUi5TextInput` template.
 *
 * furo-ui5-tag is a display-only binding (reads from the model, renders the value as slotted text,
 * never writes back), so only the read-side `[TEMPLATE]` blocks apply.
 */
import "@/Assets";
import "@/Icons";
import "./index";
...
import type { FuroUi5Tag } from "./FuroUi5Tag";
...
describe("FuroUi5Tag", () => {
  describe("element identity & a11y [TEMPLATE]", () => {
    ...
    beforeAll(async () => {
      el = await fixture(html` <furo-ui5-tag data-testid="test">label</furo-ui5-tag> `);
    ...
    it("should be a furo-ui5-tag element", () => {
      assert.equal(el.nodeName.toLowerCase(), "furo-ui5-tag");
    });
  ...
  describe("default model state [TEMPLATE]", () => {
    ...
    beforeEach(async () => {
      el = await fixture(html`<furo-ui5-tag></furo-ui5-tag>`);
    });
