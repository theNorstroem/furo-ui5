 ...
 * @appliesMixin FBP
 */
export class FuroUi5SegmentedButton extends FieldNodeAdapter(
  ...
  constructor() {
    ...
    /**
     * a list of privileged attributes. when those attributes are set in furo-ui5-select web-components initially.
   ...
   */
  readAttributes() {
    // save the original attribute for later usages, we do this, because some web-components reflect
  ...
  _updateFNA(e) {
    ...
    if (
      ...
      selectedOption = this._optionList.repeats.find(
        obj =>
          FuroUi5SegmentedButton.getValueByPath(
            obj,
      ...
      if (selectedOption) {
        newValue = FuroUi5SegmentedButton.getValueByPath(
          selectedOption,
          this._privilegedAttributes['id-field-path']
        )._value;
      }
  ...
  _updateOptions(list) {
    ...
    if (this._optionList && this._optionList.repeats) {
      this._optionList.repeats.forEach(item => {
        ...
        const DISPLAY_TEXT = FuroUi5SegmentedButton.getValueByPath(
          item,
