 ...
 * calendar-only (`YYYY-MM-DD`) for XDate.
 */
...
export class DateAndTimeReaderWriters<T> {
  ...
  private modelField: STRING | Timestamp | INT32 | INT64 | XDate;

  private valueField: StringKeys<T>;

  constructor(clazz: T, valueField: StringKeys<T>, modelField: STRING | Timestamp | INT32 | INT64 | XDate) {
    this.clazz = clazz;
