...
export class DisplayFuroFatBool extends DisplayBool {
  ...
  _getTemplate() {
    ...
    if (this._fieldValue) {
      ...
      } else if (!this._fieldValue.value || !this._fieldValue.value._value || this._fieldValue.value._value === "false") {
        tmpl = html` <ui5-icon name="border"></ui5-icon> `;
      } else {
        tmpl = html` <ui5-icon name="accept" value-state="Success"></ui5-icon> `;
      }
    }
