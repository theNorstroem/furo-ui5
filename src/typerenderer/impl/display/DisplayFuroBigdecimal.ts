  ...
  _formatDisplay() {
    ...
    if (this._field.attributes && this._field.attributes["value-state"]) {
      ...
      if (state !== "None" && this._field.attributes["value-state-message"]) {
        this.setAttribute("title", this._field.attributes["value-state-message"]._value);
      }
    }
    ...
    if (val !== null && !(val.scale === null || val.unscaled_value === null)) {
      ...
      if (val.scale < 0) {
        ...
        displayValue = parseFloat(`${vstr.substr(0, vstr.length - val.scale)}.${vstr.substr(vstr.length - val.scale)}`);
      }
      if (val !== null) {
        this._displayValue = new Intl.NumberFormat(Env.locale, this._options).format(displayValue);
      }
    }
