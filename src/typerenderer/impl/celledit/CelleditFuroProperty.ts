...
export class CelleditFuroProperty extends FBP(LitElement) {
  ...
  _waitForType(field) {
    const anyTypeHandler = () => {
      if (field.data["@type"] && field.data["@type"]._value !== undefined && this._typeResolved === false) {
        this._typeResolved = true;
        this.renderName = `celledit-${this._field.data["@type"]._value.replace(/.*\//, "").replaceAll(".", "-").toLocaleLowerCase()}`;
